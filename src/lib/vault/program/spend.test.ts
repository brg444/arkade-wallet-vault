...
import { P2A_SCRIPT_HEX, P2A_VALUE_SATS, TRANSITION_SEQUENCE, PROGRAM_CSV, WITNESS_BYTES_399 } from './constants'
...
describe('staged initiate / clawback / claim PSBTs', () => {
  ...
  it('claims serverlessly with the pending CSV and an unpinned dest', () => {
    ...
    expect(view.sequence).toBe(PROGRAM_CSV.recovery)
  ...
  it('signs and finalizes a savings hardware initiate with derived tweak keys', () => {
    ...
    expect(hex.encode(secp256k1.getPublicKey(vaultPriv, true)).slice(2)).toBe(
      tweakByArkScript(PROGRAM_FIXTURE.vaultCosignerBase, auth).slice(2),
    )
  ...
  it('signs a phone clawback and a recovery claim', () => {
    ...
    expect(inspectClaimPsbt(claim.psbtHex).sequence).toBe(PROGRAM_CSV.recovery)
  })
